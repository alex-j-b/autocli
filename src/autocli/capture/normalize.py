...
def decode_body_bytes(body: bytes | bytearray | memoryview | None, content_type: str | None) -> Any:
    ...
    if media_type == "application/x-www-form-urlencoded":
        return normalize_query(parse_qsl(body_bytes.decode(extract_charset(content_type) or "utf-8"), keep_blank_values=True))
...
def repair_flow_request_url(request: dict[str, Any], headers: dict[str, str | list[str]]) -> str:
    ...
    host_hint = (
        str(request.get("host_header") or "")
        or host_from_headers(headers)
        or str(request.get("host") or "")
    )
    if not host_hint:
        raise ValueError("unable to repair flow URL without host information")

    host, host_port = split_host_port(host_hint)
    scheme = (parsed.scheme or str(request.get("scheme") or "") or infer_scheme_from_port(coerce_int(request.get("port")) or host_port)).lower()
