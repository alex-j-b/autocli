...
def test_decode_body_bytes_handles_supported_boundary_formats() -> None:
    ...
    assert decode_body_bytes(msgpack.packb({"count": 2}, use_bin_type=True), "application/msgpack") == {
        "count": 2
    }
    ...
    assert decode_body_bytes("tere".encode("utf-8"), "text/plain; charset=utf-8") == "tere"
